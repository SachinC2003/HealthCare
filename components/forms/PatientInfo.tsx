...
const PatientInfo = ({ user }: { user: User }) => {
  ...
  const onSubmit = async (values: z.infer<typeof PatientFormValidation>) => {
    ...
    if (values.identificationDocument && values.identificationDocument.length > 0) {
      ...
      try {
        ...
        try {
          const response = await fetch("/api/uplode-img", {
            method: "POST",
            body: formData,
          });
    ...
    try {
      ...
      toast.success("Your information stoare successuffly")
    } catch (error) {
      toast.error("Error to stoare your information")
  ...
  const [identificationDocument, setIdentificationDocument] = useState<File[]>();

const onIdentificationDocumentChange = (files: File[]) => {
  setIdentificationDocument(files);
  form.setValue("identificationDocument", files);
};
  return (
    <Form {...form}>
        ...
        <section className="space-y-6">
          ...
          >
            {doctorsList.map((Doctor, i) => (
              <SelectItem key={Doctor.firstName + i} value={Doctor.firstName} >
          ...
          <div className="flex flex-col gap-6 xl:flex-row">
            ...
            <CustomFormField
              ...
              placeholder="Tylenol, Ibuprofen, Metformin"
            />
          </div>
          ...
          <div className="flex flex-col gap-6 xl:flex-row">
            <CustomFormField
              ...
              placeholder="Hypertension, Diabetes, Cancer"
            />
            ...
            <CustomFormField
              ...
              placeholder="Heart surgery, Appendectomy, Asthma"
            />
          </div>
        </section>
