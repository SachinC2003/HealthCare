...
import { sendAppointmentCanceledMassege } from '@/lib/twilio';
...
export async function PUT(request: Request) {
  try {
    ...
    const canceledAppointment = await prisma.appointment.update({
      where: { id: parseInt(appointmentId) },
      data: { status: 'canceled' },  // You could use any status that signifies cancellation
    });
    ...
    if (phone) {
      ...
      try {
        ...
        await sendAppointmentCanceledMassege("+"+phone);
      } catch (smsError) {
    ...
    return NextResponse.json({ message: 'Appointment canceled successfully', appointment: canceledAppointment });
  } catch (error) {
